<x-app-layout>
    <div class="p-6">
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            <h2 class="text-2xl font-semibold mb-4">Tours</h2>
            ...
            <div class="overflow-x-auto mt-4">
                <table id="tours-table" class="min-w-full bg-white border rounded-lg">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($tours as $tour)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4">
                                    @if ($tour->image)
                                        <img src="{{ asset('storage/' . $tour->image) }}" alt="{{ $tour->name }}" class="w-16 h-16 object-cover rounded">
                                    @else
                                        <span>No Image</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">{{ $tour->name }}</td>
                                <td class="px-6 py-4">{{ \Illuminate\Support\Str::limit($tour->description, 50) }}</td>
                                <td class="px-6 py-4">{{ $tour->destination->name ?? 'Unknown' }}</td> <!-- Display the associated destination name -->
                                <td class="px-6 py-4">
