<x-app-layout>
    <section class="content-header">
        <h1>
            Edit Package
            <small>Edit the travel package details</small>
        </h1>
        <ol class="breadcrumb">
                    ...
                    <form action="{{ route('packages.update', $package->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" id="name" class="form-control" value="{{ old('name', $package->name) }}" placeholder="Enter Package Name" required>
                            </div>

                            <div class="form-group">
                                <label for="duration">Duration in days</label>
                                <input type="number" name="duration" id="duration" class="form-control" value="{{ old('duration', $package->duration) }}" placeholder="Enter Package Duration" required min="0">
                            </div>
                            ...
                            <div>
                                <textarea class="textarea" placeholder="Description" name="description" id="editor"
                                          style="width: 100%; height: 125px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;">{{ old('description', $package->description) }}</textarea>
                            </div>

                            <div class="form-group">
                                <label for="image">Image</label>
                                <input type="file" name="image" id="image" class="form-control" accept="image/*" onchange="previewImage(event)">
                                <div id="image-preview-container" style="display: {{ $package->image ? 'block' : 'none' }}; margin-top: 10px;">
                                    <img id="image-preview" class="img-thumbnail" style="max-width: 200px;" src="{{ $package->image ? asset('storage/' . $package->image) : '' }}" alt="Image Preview">
                                </div>
                            </div>
                        </div>
                        <!-- /.box-body -->
